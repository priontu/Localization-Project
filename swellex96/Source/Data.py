...
class VLA:
    ...
    def __init__(self) -> None:
        self.curr_loc = __file__.split("Data")[0].replace("\\", "/")
    ...
    def plot(self, limit = defaults.default_point_limit, channel = defaults.default_plot_channel):
...
class TLA:
    ...
    def plot(self, limit = defaults.default_point_limit, channel = defaults.default_plot_channel):
        data_series = self.export_df()
        data_series.iloc[0:limit, :].plot(x = "time", y = "channel_" + str(channel))
        plt.show()
